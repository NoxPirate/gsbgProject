...
# -------------------- Load Model --------------------
# Support multiple saved formats:
# - a single estimator object with predict_proba
# - a tuple (vectorizer, estimator) created by train.py
vectorizer = None
estimator = None
if os.path.exists(MODEL_PATH):
    ...
    if isinstance(loaded, (list, tuple)) and len(loaded) == 2:
        vectorizer, estimator = loaded
    else:
        # assume the loaded object is an estimator which itself handles text
        estimator = loaded
else:
    estimator = None
...
def ml_score(msg: str):
    try:
        text = clean_text(msg)
        # If we have a vectorizer + estimator pair (expected from train.py), use it.
        if vectorizer is not None and estimator is not None:
            X = vectorizer.transform([text])
            probs = estimator.predict_proba(X)[0]
            labels = estimator.classes_
        elif estimator is not None:
            # assume estimator can handle raw text
            probs = estimator.predict_proba([text])[0]
            labels = estimator.classes_
        else:
            # no model available
